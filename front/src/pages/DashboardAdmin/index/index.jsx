...
export default function index() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            await api.get( urlPaginate?urlPaginate:`/api/admin/dashboard`,{ headers: { 'Authorization': `Bearer ${token}` }}).then((response) => {
            ...
            }).catch(function (err) {err.response.data.error === "Unauthorized" &&  navigate("/login") });
                ...
                <div className="w-screen p-3 ">
                    ...
                    <div className="p-5 flex flex justify-between items-center mb-4">
                        ...
                        <div className="flex gap-2">
                            <button onClick={()=>setFilter('Em Aberto')} className="border-red-600 text-red-600  border-2 rounded p-1">
                                Em Aberto
                            </button>
                            <button  onClick={()=>setFilter('Em Andamento')} className="border-yellow-600 text-yellow-600  border-2 rounded  p-1">
                                Em Andamento
                            </button>
                            <button onClick={()=>setFilter('Concluido')} className="border-green-600 text-green-600  border-2 rounded  p-1">
                                Concluido
                            </button>
                        </div>
                    </div>

                    <div className="relative overflow-x-auto h-max drop-shadow-md">
                                ...
                                {calleds.calleds &&
                                    ...
                                    calleds.calleds.map((item) => (
                                        <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                            ...
                                            <td className="px-6 py-4 text-center " onClick={()=>called({
